...
%{__install} -m 755 -d %{buildroot}/%{home}/bin %{buildroot}/%{home}/lib %{buildroot}/%{home}/conf %{buildroot}/%{home}/extlib %{buildroot}/%{home}/plugins %{buildroot}/%{logs}
%{__ln_s} -f %{logs} %{buildroot}/%{home}/log

# Install  binaries, libraries and configuration files
cp azkaban/azkaban-exec-server/build/install/azkaban-exec-server/bin/* %{buildroot}/%{home}/bin/
cp azkaban/azkaban-exec-server/build/install/azkaban-exec-server/lib/* %{buildroot}/%{home}/lib/
%{__install} -m 644 -D rpm/azkaban-executor/azkaban.properties %{buildroot}/%{home}/conf/
