...
%description
  This package provides Azkaban solo service.
...
%{__install} -m 755 -d %{buildroot}/%{home}/bin %{buildroot}/%{home}/lib %{buildroot}/%{home}/conf %{buildroot}/%{home}/extlib %{buildroot}/%{home}/plugins %{buildroot}/%{logs} %{buildroot}/%{home}/web %{buildroot}/%{home}/sql
%{__ln_s} -f %{logs} %{buildroot}/%{home}/log

# Install  binaries, libraries and configuration files
cp azkaban/azkaban-solo-server/build/install/azkaban-solo-server/bin/* %{buildroot}/%{home}/bin/
cp azkaban/azkaban-solo-server/build/install/azkaban-solo-server/lib/* %{buildroot}/%{home}/lib/
cp -r azkaban/azkaban-solo-server/build/install/azkaban-solo-server/web/* %{buildroot}/%{home}/web/
cp azkaban/azkaban-solo-server/build/install/azkaban-solo-server/sql/* %{buildroot}/%{home}/sql/
cp azkaban/azkaban-solo-server/build/install/azkaban-solo-server/conf/* %{buildroot}/%{home}/conf/
cp -r azkaban/azkaban-solo-server/build/install/azkaban-solo-server/plugins/* %{buildroot}/%{home}/plugins/
